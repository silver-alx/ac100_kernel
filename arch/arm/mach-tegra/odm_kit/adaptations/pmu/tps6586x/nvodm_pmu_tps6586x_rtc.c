...
static unsigned long epoch_end_sec = 0;
    ...
    NvU32 Count)
{
    ...
    if (!epoch_sec)
        epoch_sec = mktime(epoch,1,1,0,0,0);
    if(!epoch_end_sec)
        epoch_end_sec = mktime(epoch_end,12,31,23,59,59);

    if (Count < (NvU32)epoch_sec || Count > epoch_end_sec)
    {
        // prevent setting date earlier than 'epoch'
        pr_warning("\n Date must being set in range from 2009.1.1 00:00:00 to 2037.12.31 23:59:59 \n");
