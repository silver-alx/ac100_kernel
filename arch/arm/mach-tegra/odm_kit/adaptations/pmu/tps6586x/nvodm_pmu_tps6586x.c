...
//    static TPS6586xDevice s_tps6586x = {0};
    ...
    if (pConnectivity != NULL) // PMU is in database
    {
        ...
        if (NV_FALSE == Tps6586xWriteVoltageReg(hDevice, TPS6586xPmuSupply_LDO5, tps6586xSupplyInfoTable[TPS6586xPmuSupply_LDO5].cap.requestMilliVolts, NULL))
            NVODMPMU_PRINTF(("TPS: Fail to set the VCORE_MMC to 2.85V\n"));
        else
            NVODMPMU_PRINTF(("TPS: set the VCORE_MMC to 2.85V\n"));
    }
    else
    {
